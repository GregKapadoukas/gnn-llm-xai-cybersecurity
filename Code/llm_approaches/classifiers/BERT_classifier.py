...
# Please use the commented code to switch from IoT23 to Edge-IIoTset datasets
os.environ["TOKENIZERS_PARALLELISM"] = "false"
###Get the graphics card###
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(device)
###Load Data###
sequences = pd.read_csv("../../../Datasets/IOT-23/CSVs/IoT23_sequences_multi.csv")
# sequences = pd.read_csv('../../../Datasets/Edge-IIoTset Cyber Security Dataset of IoT & IIoT/CSVs/EdgeIIoT__sequences_multi.csv')
###shuffle###
...
sequences_train, sequences_test = train_test_split(
    sequences, test_size=0.1, stratify=sequences["label"], random_state=42, shuffle=True
)
sequences_train, sequences_validation = train_test_split(
    sequences_train,
    test_size=0.1111,
    stratify=sequences_train["label"],
    random_state=42,
    shuffle=True,
)
#####
...
def compute_metrics(eval_pred):
    ...
    other_metrics_scores = other_metrics.compute(
...
def compute_metrics_test(eval_pred):
    predictions, labels = eval_pred
    predictions = np.argmax(predictions, axis=-1)
    matrix = confusion_matrix.compute(references=labels, predictions=predictions)[
        "confusion_matrix"
    ]
    matrix = pd.DataFrame(matrix, index=ground_truth, columns=ground_truth)
    matrix.to_csv("../../../Checkpoints/IoT23/multi/IoT23_confusion_multi.csv")
    # matrix.to_csv("../../../Checkpoints/Edge/multi/EdgeIIoT_confusion_multi.csv")
    other_metrics_scores = other_metrics.compute(
        predictions=predictions, references=labels, average=None
    )
    accuracy_score = accuracy.compute(predictions=predictions, references=labels)[
...
class SparseCategoricalFocalLoss(nn.Module):
    ...
    def forward(self, logits, labels):
        ...
        loss = (
            -alpha_factor
            * ((1 - true_propabilities) ** self.gamma)
            * torch.log(true_propabilities + 1e-8)
        )
        #
...
class ImbalancedTrainer(Trainer):
    def __init__(self, *args, loss_fn=None, **kwargs):
        ...
        self.loss_fn = SparseCategoricalFocalLoss(
            gamma=2, alpha=weights, reduction="mean"
        )
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        logits = outputs.get(
            "logits"
        )  # get the model's output(logits) for these inputs
...
training_arguments = TrainingArguments(
    output_dir="../../../Checkpoints/run",  # Location where the fine tuned model's weights will be stored
...
results = trainer.evaluate(
    eval_dataset=sequences_test
)  # Evaluate on unseen test subset
print(results)
###Save the model###
tokenizer.save_pretrained(
    "../../../Checkpoints/saved_tokenizers/IoT23/multi"
)  # save the tokenizer
# tokenizer.save_pretrained(
#    "../../../Checkpoints/saved_tokenizers/Edge/multi"
# )  # save the tokenizer
model.config.save_pretrained(
    "../../../Checkpoints/saved_models/IoT23/multi"
)  # save the base model's config such as id2label etc
# model.config.save_pretrained(
#    "../../../Checkpoints/saved_models/Edge/multi"
# )  # save the base model's config such as id2label etc

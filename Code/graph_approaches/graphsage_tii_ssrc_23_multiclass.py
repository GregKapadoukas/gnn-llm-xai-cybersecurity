...
attack_classification_best_model_params_path = os.path.join(
    "../../Checkpoints/",
    f"graphsage-tii-ssrc-23-multiclass-{number_nodes}-{aggregator_type}-{embedding_size}.pt",
)
...
evaluation_mode = {
    "mode": "train-test-dev",
    "set": "train",
    "name": "graphsage-tii-ssrc-23-multiclass",
}

attack_classification_model = GraphSAGE(
    number_nodes=number_nodes,
    number_features=4,
    hidden_features=embedding_size,
    batch_size=batch_size,
    dropout=0.5,
    num_classes=4,
    aggregator_type=aggregator_type,  # sum, mean, or max
    device=device,
).to(device)
...
with open(
    "Results/Pickle/graphsage-tii-ssrc-23-multiclass-results-dev.pkl",
    "wb",
) as file:
...
with open(
    "Results/Pickle/graphsage-tii-ssrc-23-multiclass-results-test.pkl",
    "wb",
) as file:
    pickle.dump(results, file)
...
print("=" * 89)
with open(
    "Results/Pickle/graphsage-tii-ssrc-23-multiclass-results-dev.pkl", "rb"
) as file:
...
print("=" * 89)
with open(
    "Results/Pickle/graphsage-tii-ssrc-23-multiclass-results-test.pkl", "rb"
) as file:
    results = pickle.load(file)
print(
    f"| accuracy: {results['accuracy']} "

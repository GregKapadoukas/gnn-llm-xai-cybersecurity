...
batch_size = 512
embedding_size = 80 * 2
aggregator_type = "sum"  # sum, mean, or max
...
attack_classification_best_model_params_path = os.path.join(
    "../../Checkpoints/",
    f"gin-tii-ssrc-23-multiclass-{number_nodes}-{aggregator_type}-{embedding_size}.pt",
)
...
evaluation_mode = {
    "mode": "train-test-dev",
    "set": "train",
    "name": "gin-tii-ssrc-23-multiclass",
}

attack_classification_model = GIN(
...
with open(
    "Results/Pickle/gin-tii-ssrc-23-multiclass-results-dev.pkl",
    "wb",
) as file:
...
with open(
    "Results/Pickle/gin-tii-ssrc-23-multiclass-results-test.pkl",
    "wb",
) as file:
    pickle.dump(results, file)
...
print("=" * 89)
with open("Results/Pickle/gin-tii-ssrc-23-multiclass-results-dev.pkl", "rb") as file:
...
print("=" * 89)
with open("Results/Pickle/gin-tii-ssrc-23-multiclass-results-test.pkl", "rb") as file:
    results = pickle.load(file)
print(
    f"| accuracy: {results['accuracy']} "
    f"| macro precision: {results['precision']}\n"

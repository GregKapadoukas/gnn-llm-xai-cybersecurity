...
from graph_dataset.graph_dataset import (
    loadDynamicGraphDataset,
    oversampleInfrequentClasses,
    randomizeGraphOrder,
    splitGraphDataset,
)
from preprocessing.convert_to_dynamic_graphs import convertToDynamicGraphs
...
loadCSVsAndCreateGraphs(
    csvs_paths, graphs_path, number_nodes, 10000, "Generalized", "all"
)
...
dynamic_graphs_path = f"../../Datasets/TII-SSRC-23 Dataset/My Preprocessing/Dynamic Graphs/Size {num_time_steps}*{number_nodes}/"
convertToDynamicGraphs(graphs_path, dynamic_graphs_path, num_time_steps)
# -
...
dynamic_graphs_path = f"../../Datasets/TII-SSRC-23 Dataset/My Preprocessing/Dynamic Graphs/Size {num_time_steps}*{number_nodes}/"
(graphs, labels) = loadDynamicGraphDataset(dynamic_graphs_path, one_hot_mapping, 10)
(
    graphs_train,
    graphs_dev,
    graphs_test,
...
attack_classification_best_model_params_path = os.path.join(
    "../../Checkpoints/",
    f"tii-ssrc-23-multiclass-temporal-gnn-{number_nodes}-{number_eigenvectors}-{embedding_size}.pt",
)
...
evaluation_mode = {
    "mode": "train-test-dev",
    "set": "train",
    "name": "tii-ssrc-23-multiclass-temporal-gcn",
}

attack_classification_model = TemporalGNN(
...
results = evaluate(
    attack_classification_model,
    attack_classification_best_model_params_path[:-3],
    loss_function,
    graphs_dev,
    labels_dev,
...
with open(
    "Results/Pickle/tii-ssrc-23-multiclass-temporal-gnn-results-dev.pkl",
    "wb",
) as file:
...
with open(
    "Results/Pickle/tii-ssrc-23-multiclass-temporal-gnn-results-test.pkl",
    "wb",
) as file:
    pickle.dump(results, file)
